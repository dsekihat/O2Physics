...
struct skimmerOTS {
  ...
  void initCCDB(TBC const& bc)
  {
    ...
    for (size_t idx = 0; idx < mTOIidx.size(); idx++) {
      ...
      LOGF(info, "Trigger of Interest : index = %d, scaler = %llu, selection = %llu", idx, mScalers[idx], mSelections[idx]);
    }
  ...
  void process(MyCollisions const& collisions, MyBCs const&)
  {
    for (const auto& collision : collisions) {
      ...
      uint16_t analyzed_bitmap = 0; // this is identical to trigger_bitmap;
      ...
      if (zorro.isSelected(bc.globalBC(), bcMarginForSoftwareTrigger)) { // triggered event
        ...
        for (size_t idx = 0; idx < mTOIidx.size(); idx++) {
          if (swt_bitset.test(mTOIidx[idx])) {
            ...
            if (ATcounters[idx] > mATCounters[emswtId]) {
              analyzed_bitmap |= BIT(emswtId);
              mATCounters[emswtId] = ATcounters[idx];
            }

            if (TOIcounters[idx] > mTOICounters[emswtId]) {
              analyzedToI_bitmap |= BIT(emswtId);
              mTOICounters[emswtId] = TOIcounters[idx];
            }

            // LOGF(info, "collision.globalIndex() = %d, bc.globalBC() = %llu, TOIcounters[%d] = %d, mTOICounters[%d] = %d, mATcounters[%d] = %d", collision.globalIndex(), bc.globalBC(), emswtId, TOIcounters[emswtId], emswtId, mTOICounters[emswtId], emswtId, mATCounters[emswtId]);
          }
        } // end of TOI loop
      }
